...
function App() {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      setSidebarWidth(Math.max(240, Math.min(newWidth, 640)));
    };
 ...
 <button onClick={(e) => {
        ...
        className={`px-1.5 py-0.5 border transition-all flex items-center justify-center shrink-0 ${ isHidden ? 'bg-red-500/20 text-red-400 border-red-500/30' : 'bg-[#1e252e] text-[#8a95a3] border-[#1e252e] hover:border-[#4e5d74] hover:text-[#00ffff]'}`}
 ...
 <button onClick={(e) => {
        ...
        className={`px-2 py-0.5 text-[9px] font-bold border transition-all flex items-center justify-center shrink-0 gap-1 ${ isSelected ? 'bg-[#00ffff] text-black border-[#00ffff] shadow-[0_0_10px_rgba(0,255,255,0.5)]' : 'bg-[#1e252e] text-[#8a95a3] border-[#1e252e] hover:border-[#4e5d74]'}`}>
...
return (
    <div className={`w-full h-screen flex ${ sidebarPosition === 'right' ? 'flex-row-reverse' : 'flex-row'} theme-${ themeMode} bg-[#080a0d] text-[#8a95a3] font-sans overflow-hidden`}>
      {/* Editor Sidebar */}
      <div
        className={`h-full flex flex-col bg-[#111418] z-20 shrink-0 relative ${ sidebarPosition === 'right' ? 'border-l border-[#1e252e]' : 'border-r border-[#1e252e]'} ${ isResizingSidebar ? '' : 'transition-all duration-300 ease-in-out'}`}
        style={{
        ...
        {isSidebarOpen &&
        <div
          className={`absolute top-0 bottom-0 w-2 cursor-col-resize z-50 hover:bg-[#00ffff]/20 ${ sidebarPosition === 'left' ? '-right-1' : '-left-1'}`}
          onMouseDown={(e) => {
            e.preventDefault();
        ...
        <div className="h-full flex flex-col shrink-0 relative overflow-hidden" style={{ width: `${ sidebarWidth}px` }}>
          <div className="h-[100px] px-6 border-b border-[#1e252e] shrink-0 bg-[#0a0c10] flex flex-col justify-center gap-1.5">
            <div className="flex items-center gap-3">
              <div className="border border-[#4e5d74] p-1  shrink-0 bg-[#111418]">
          ...
          <div className="flex border-b border-[#1e252e] bg-[#080a0d] px-5 py-3 shrink-0">
            <div className="flex items-center gap-1 bg-[#111418] border border-[#1e252e] p-1 w-full">
 <button className={`flex-1 py-1.5 text-[10px] font-bold tracking-widest transition-all ${ orientation === 'horizontal' ? 'bg-[#2d3640] text-[#00ffff] shadow-sm' : 'text-[#8a95a3] hover:text-[#e2e8f0]'}`} onClick={() => setOrientation('horizontal')}
              title="横組レギュラー">
                {lang === 'jp' ? '☰ 横組' : '☰ REGULAR'}</button>
 <button className={`flex-1 py-1.5 text-[10px] font-bold tracking-widest transition-all ${ orientation === 'vertical' ? 'bg-[#2d3640] text-[#00ffff] shadow-sm' : 'text-[#8a95a3] hover:text-[#e2e8f0]'}`} onClick={() => setOrientation('vertical')}
              title="縦組リール">
 ...
 <button className={`flex-1 py-2 text-[10px] font-bold tracking-widest transition-all ${ activeTab === 'design' ? 'bg-[#1a1f26] text-white shadow-sm border border-[#2d3640]' : 'text-[#8a95a3] hover:text-[#e2e8f0] border border-transparent'}`} onClick={() => setActiveTab('design')}>
                {lang === 'jp' ? 'デザイン' : 'DESIGN'}</button>
 <button className={`flex-1 py-2 text-[10px] font-bold tracking-widest transition-all ${ activeTab === 'image' ? 'bg-[#1a1f26] text-white shadow-sm border border-[#2d3640]' : 'text-[#8a95a3] hover:text-[#e2e8f0] border border-transparent'}`} onClick={() => setActiveTab('image')}>
                {lang === 'jp' ? '画像' : 'IMAGE'}</button>
 <button className={`flex-1 py-2 text-[10px] font-bold tracking-widest transition-all ${ activeTab === 'text' ? 'bg-[#1a1f26] text-white shadow-sm border border-[#2d3640]' : 'text-[#8a95a3] hover:text-[#e2e8f0] border border-transparent'}`} onClick={() => setActiveTab('text')}>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {/* IMAGE 1 */}
                  <div className="bg-[#111418] border border-[#1e252e] p-3  flex flex-col gap-3">
 ...
 {imageUrl && <button onClick={() => setImageUrl('')} className="hover:text-red-400"><Trash2 size={10} /></button>}
                    ...
                    <label
                      className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-[#1e252e]  p-2 bg-[#080a0d] hover:bg-[#111418] hover:border-[#4e5d74] transition-all cursor-pointer relative overflow-hidden group min-h-[60px]"
 ...
 {image2Url && <button onClick={() => setImage2Url('')} className="hover:text-red-400"><Trash2 size={10} /></button>}
              ...
              <div className="space-y-4 pt-6 border-t border-[#1e252e]">
                ...
                <div className="grid grid-cols-1 gap-3">
                   <div className="bg-[#111418] border border-[#1e252e] p-3  flex flex-col justify-center items-center gap-3">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="col-span-2">
                    ...
                    <input type="text" className="ss-input  w-full" value={kicker} onChange={(e) => setKicker(e.target.value)} />
                    {/* renderBlockSettings removed */}
                  </div>
                  <div className="col-span-2">
                    <div className="text-[9px] mb-1.5 font-bold tracking-widest opacity-60 flex items-center justify-between">
                    ...
                    <textarea className="ss-input h-16 resize-none  w-full" value={heading} onChange={(e) => setHeading(e.target.value)} />
                    {/* renderBlockSettings removed */}
                  </div>
                  <div className="col-span-2">
                    <div className="text-[9px] mb-1.5 font-bold tracking-widest opacity-60 flex items-center justify-between">
                    ...
                    <textarea className="ss-input h-24 resize-none  leading-relaxed text-[11px] w-full" value={body} onChange={(e) => setBody(e.target.value)} />
                    {/* renderBlockSettings removed */}
                  </div>
                  <div className="col-span-2">
                    <div className="text-[9px] mb-1.5 font-bold tracking-widest opacity-60 flex items-center justify-between">
                    ...
                    <textarea className="ss-input h-24 resize-none  leading-relaxed text-[11px] placeholder:text-[#2d3a4d]/50 w-full" placeholder="Optional second body text block..." value={body2} onChange={(e) => setBody2(e.target.value)} />
                    {/* renderBlockSettings removed */}
                  </div>
                  <div className="col-span-2 sm:col-span-1">
                    ...
                    <input type="text" className="ss-input  w-full" value={meta1} onChange={(e) => setMeta1(e.target.value)} />
                    {/* renderBlockSettings removed */}
                  </div>
                  <div className="col-span-2 sm:col-span-1">
                    <div className="text-[9px] mb-1.5 font-bold tracking-widest opacity-60 flex items-center justify-between">
                    ...
                    <input type="text" className="ss-input  w-full" value={meta2} onChange={(e) => setMeta2(e.target.value)} />
                    {/* renderBlockSettings removed */}
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex gap-2">
 <button className={`px-1.5 py-1 text-[9px] font-bold tracking-widest transition-all flex items-center justify-center gap-1 border ${ isMonotone ? 'bg-blue-600/20 text-blue-400 border-blue-500/30' : 'bg-[#080a0d] text-[#8a95a3] hover:text-[#e2e8f0] border-[#1e252e]'}`} onClick={() => setIsMonotone(!isMonotone)}>
 ...
 <button onClick={() => setStylePattern(f.id)}
                    className={`w-full flex flex-col p-2.5 border transition-all text-left ${
                    stylePattern === f.id ?
 ...
 <button className={`absolute top-1.5 right-1.5 p-1 transition-all opacity-0 group-hover:opacity-100 bg-[#0a0c10] border border-[#1e252e] hover:border-[#4d5e7a] text-[#8a95a3] hover:text-[#00ffff]`} title={lang === 'jp' ? '配置とスタイルをリセット' : 'Reset Offset & Style'} onClick={(e) => {
                      e.stopPropagation();
 ...
 <button className={`flex-1 py-1.5 text-[10px] font-bold tracking-widest transition-all flex items-center justify-center gap-1.5 border border-[#1e252e] bg-[#080a0d] hover:bg-[#1e252e] text-[#8a95a3]`} onClick={() => {
                    setOffsets((prev) => ({ ...prev, [`${ stylePattern}-${ orientation}`]: {} }));
                  }}>
                    
                    <RotateCcw size={12} /> {lang === 'jp' ? '配置リセット' : 'UN-OFFSET'}
 </button>
 <button className={`flex-1 py-1.5 text-[10px] font-bold tracking-widest transition-all flex items-center justify-center gap-1.5 border border-[#1e252e] bg-[#080a0d] hover:bg-[#1e252e] text-[#8a95a3] hover:text-[#00ffff]`} onClick={() => {
 ...
 <button className={`px-2 py-0.5 text-[9px] font-bold border transition-all ${ gridMode !== 'none' ? 'bg-[#00ffff] text-black border-[#00ffff] shadow-[0_0_10px_rgba(0,255,255,0.5)]' : 'bg-[#1e252e] text-[#8a95a3] border-[#1e252e] hover:border-[#4e5d74]'}`} onClick={() => setGridMode(gridMode === 'none' ? gridColor : 'none')}>
                    
                    {gridMode !== 'none' ? 'ON' : 'OFF'}
 </button>
                </div>

                <div className="flex bg-[#080a0d] p-1.5 border border-[#1e252e] gap-1.5 flex-wrap">
 <button className={`flex-1 min-w-[50px] py-1.5 text-[10px] font-bold tracking-widest transition-all flex items-center justify-center gap-1.5 ${ gridColor === 'cyan' ? 'bg-[#2d3640] text-[#00ffff] shadow-sm' : 'text-[#8a95a3] hover:text-[#e2e8f0]'}`} onClick={() => {
                    setGridColor('cyan');
 ...
 <button className={`flex-1 min-w-[50px] py-1.5 text-[10px] font-bold tracking-widest transition-all flex items-center justify-center gap-1.5 ${ gridColor === 'dark' ? 'bg-[#2d3640] text-white shadow-sm' : 'text-[#8a95a3] hover:text-[#e2e8f0]'}`} onClick={() => {
                    setGridColor('dark');
 ...
 <button className={`flex-1 min-w-[50px] py-1.5 text-[10px] font-bold tracking-widest transition-all flex items-center justify-center gap-1.5 ${ gridColor === 'light' ? 'bg-[#2d3640] text-white shadow-sm' : 'text-[#8a95a3] hover:text-[#e2e8f0]'}`} onClick={() => {
                    setGridColor('light');
              ...
              <div className="space-y-4 pt-6 border-t border-[#1e252e]">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[1, 2, 3, 4].map((slot) => {
                    ...
                    return (
                      <div key={slot} className="flex flex-col bg-[#1a1f26] border border-[#1e252e] overflow-hidden relative group h-[60px]">
                        {isSaved ?
                        <div className="flex flex-col h-full">
 <button className="flex-1 text-[10px] font-bold tracking-widest text-[#00ffff] hover:bg-[#2d3640] transition-colors flex items-center justify-center pt-1" onClick={() => loadSlot(slot)}>
                            
                               LOAD SLOT {slot}
 </button>
                             <div className="flex border-t border-[#1e252e] h-[24px] shrink-0">
 <button className="flex-1 text-[9px] font-bold tracking-widest text-[#8a95a3] hover:text-white border-r border-[#1e252e] hover:bg-[#4e5d74] transition-colors flex items-center justify-center gap-1" onClick={() => {
                              const data = { imageUrl, image2Url, kicker, heading, body, body2, meta1, meta2, orientation, stylePattern, gridMode, isMonotone, blockStyles, offsets };
 ...
 <button className="flex-1 text-[9px] font-bold tracking-widest text-[#8a95a3] hover:text-white hover:bg-red-900 transition-colors flex items-center justify-center gap-1" onClick={() => {
                        ...
                        <button className="h-full w-full text-[10px] font-bold tracking-widest text-[#8a95a3] hover:text-[#e2e8f0] hover:bg-[#2d3640] transition-colors flex flex-col items-center justify-center gap-1" onClick={() => {
                          const data = { imageUrl, image2Url, kicker, heading, body, body2, meta1, meta2, orientation, stylePattern, gridMode, isMonotone, blockStyles, offsets };
                          localStorage.setItem(`solid-design-slot-${ slot}`, JSON.stringify(data));
 ...
 <button className={`col-span-1 py-1.5 text-[10px] font-bold tracking-widest  transition-all flex items-center justify-center gap-1.5 border border-[#1e252e] ${ isSaving ? 'bg-blue-600 text-white' : 'bg-[#080a0d] hover:bg-[#1e252e] text-[#8a95a3]'}`} onClick={handleManualSave}>
              <Save size={12} /> {lang === 'jp' ? '一時保存' : 'SAVE SLOT 1'}
 </button>
 <button disabled={isDownloading} className={`col-span-1 py-1.5 text-[10px] font-bold tracking-widest  transition-all flex items-center justify-center gap-1.5 border ${isDownloading ? 'border-[#00ffff] bg-[#00ffff]/20 text-[#00ffff]' : 'border-[#2d3640] bg-[#111418] hover:bg-[#1e252e] text-white'}`} onClick={handleDownload}>
              {isDownloading ? <span className="animate-pulse">{lang === 'jp' ? '処理中...' : 'PROCESSING...'}</span> : <><Download size={12} /> {lang === 'jp' ? '画像DL' : 'DL IMAGE'}</>}
 </button>
          </div>
          <div className="grid grid-cols-2 gap-1.5">
 <button className="col-span-1 py-1.5 text-[10px] font-bold tracking-widest  transition-all flex items-center justify-center gap-1.5 border border-[#1e252e] bg-[#080a0d] hover:bg-[#1e252e] text-[#8a95a3]" onClick={() => fileInputRef.current?.click()}>
 ...
 <button className="col-span-1 py-1.5 text-[10px] font-bold tracking-widest  transition-all flex items-center justify-center gap-1.5 border border-[#1e252e] bg-[#080a0d] hover:bg-[#1e252e] text-[#8a95a3]" onClick={handleExportProject}>
            ...
            <div className="p-5 flex-1 overflow-y-auto space-y-6">
              <div>
                <div className="flex justify-between items-end mb-2">
                  <div className="text-[9px] font-bold tracking-widest text-[#4e5d74] uppercase">{lang === 'jp' ? 'サイドバーの幅' : 'Sidebar Width'}</div>
                  <div className="text-[10px] font-bold text-[#00ffff]">{sidebarWidth}px</div>
                </div>
                <input
                  type="range"
                  min="240"
              ...
              <div>
                ...
                <div className="flex bg-[#111418] border border-[#1e252e] text-[10px] font-bold p-1">
 <button onClick={() => setLang('en')} className={`flex-1 py-2 transition-colors ${ lang === 'en' ? 'bg-[#2d3640] text-[#00ffff]' : 'text-[#8a95a3] hover:text-[#e2e8f0]'}`}>ENGLISH</button>
 <button onClick={() => setLang('jp')} className={`flex-1 py-2 transition-colors ${ lang === 'jp' ? 'bg-[#2d3640] text-[#00ffff]' : 'text-[#8a95a3] hover:text-[#e2e8f0]'}`}>日本語</button>
                </div>
              </div>
              <div>
                <div className="text-[9px] font-bold tracking-widest text-[#4e5d74] mb-2 uppercase">{lang === 'jp' ? 'キャンバス背景色' : 'Canvas Background'}</div>
                <div className="flex items-center gap-3">
                  <div className="relative w-8 h-8  overflow-hidden border-2 border-[#1e252e] shrink-0">
                    <input
                      type="color"
                      value={canvasBgColor}
                  ...
                  <div className="flex-1">
                    <input
                      ...
                      value={canvasBgColor}
                      onChange={(e) => setCanvasBgColor(e.target.value)}
                      className="w-full bg-[#0a0c10] border border-[#1e252e] px-3 py-2 text-[10px] font-bold tracking-widest text-[#8a95a3] focus:outline-none focus:border-[#00ffff]" />
                    
                  </div>
                  <div className="flex gap-1 shrink-0">
 <button onClick={() => setCanvasBgColor('#151515')} className="w-8 h-8 bg-[#151515] border border-[#1e252e] hover:border-[#4e5d74] transition-colors" title="Dark setting"></button>
 <button onClick={() => setCanvasBgColor('#808080')} className="w-8 h-8 bg-[#808080] border border-[#1e252e] hover:border-[#4e5d74] transition-colors" title="Gray setting"></button>
 <button onClick={() => setCanvasBgColor('#ffffff')} className="w-8 h-8 bg-[#ffffff] border border-[#1e252e] hover:border-[#4e5d74] transition-colors" title="White setting"></button>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
 <button onClick={() => setArtboardShadow(true)} className={`flex-1 py-2 text-[10px] font-bold border ${ artboardShadow ? 'bg-[#2d3640] text-[#00ffff] border-[#4e5d74]' : 'bg-[#0a0c10] text-[#8a95a3] border-[#1e252e] hover:text-[#e2e8f0]'}`}>{lang === 'jp' ? 'オン' : 'ON'}</button>
 <button onClick={() => setArtboardShadow(false)} className={`flex-1 py-2 text-[10px] font-bold border ${!artboardShadow ? 'bg-[#2d3640] text-[#00ffff] border-[#4e5d74]' : 'bg-[#0a0c10] text-[#8a95a3] border-[#1e252e] hover:text-[#e2e8f0]'}`}>{lang === 'jp' ? 'オフ' : 'OFF'}</button>
                </div>
              </div>
              <div>
                <div className="flex justify-between items-end mb-2">
 ...
 <button onClick={() => setShowStatusText(true)} className={`flex-1 py-2 text-[10px] font-bold border ${ showStatusText ? 'bg-[#2d3640] text-[#00ffff] border-[#4e5d74]' : 'bg-[#0a0c10] text-[#8a95a3] border-[#1e252e] hover:text-[#e2e8f0]'}`}>{lang === 'jp' ? '表示' : 'SHOW'}</button>
 <button onClick={() => setShowStatusText(false)} className={`flex-1 py-2 text-[10px] font-bold border ${!showStatusText ? 'bg-[#2d3640] text-[#00ffff] border-[#4e5d74]' : 'bg-[#0a0c10] text-[#8a95a3] border-[#1e252e] hover:text-[#e2e8f0]'}`}>{lang === 'jp' ? '非表示' : 'HIDE'}</button>
                </div>
                {showStatusText &&
                <div className="flex flex-col gap-5 mt-5 mb-2">
                    ...
                    <div className="flex gap-2">
 <button onClick={() => setStatusTheme('dark')} className={`flex-1 py-2 text-[10px] font-bold border ${ statusTheme === 'dark' ? 'bg-[#2d3640] text-[#00ffff] border-[#4e5d74]' : 'bg-[#0a0c10] text-[#8a95a3] border-[#1e252e] hover:text-[#e2e8f0]'}`}>{lang === 'jp' ? 'ダーク' : 'DARK'}</button>
 <button onClick={() => setStatusTheme('light')} className={`flex-1 py-2 text-[10px] font-bold border ${ statusTheme === 'light' ? 'bg-[#2d3640] text-[#00ffff] border-[#4e5d74]' : 'bg-[#0a0c10] text-[#8a95a3] border-[#1e252e] hover:text-[#e2e8f0]'}`}>{lang === 'jp' ? 'ライト' : 'LIGHT'}</button>
                    </div>
                  </div>
                }
              </div>
                 ...
                 <div className="flex items-center gap-2">
                    ...
                    <motion.div
                      ...
                      className="mr-2">
                          ...
                          <div className="h-[28px] px-3 text-[9px] font-bold tracking-widest flex items-center gap-2  bg-[#111418] border border-[#2d3640] text-[#00ffff] shadow-sm">
                            <span className="w-1.5 h-1.5  animate-pulse bg-[#00ffff]"></span>
                    ...
                    <div className="flex bg-[#111418] border border-[#1e252e] text-[9px] font-bold p-0.5 h-[28px]">
 <button onClick={() => setIsEditMode(true)} className={`px-3 h-full flex items-center justify-center transition-colors ${ isEditMode ? 'bg-[#2d3640] text-[#00ffff]' : 'text-[#8a95a3] hover:text-[#e2e8f0]'}`}>{lang === 'jp' ? '編集' : 'EDIT'}</button>
 <button onClick={() => {setIsEditMode(false);setSelectedBlockId(null);}} className={`px-3 h-full flex items-center justify-center transition-colors ${!isEditMode ? 'bg-[#2d3640] text-[#00ffff]' : 'text-[#8a95a3] hover:text-[#e2e8f0]'}`}>{lang === 'jp' ? 'プレビュー' : 'PREVIEW'}</button>
                    </div>
                 </div>
                 ...
                 <div className="flex items-center gap-3 border-l border-[#1e252e] pl-6">
                    ...
                    className="w-[100px] accent-[#00ffff] h-1 bg-[#1e252e]  " />
                 ...
                 <div className="flex items-center gap-2 border-l border-[#1e252e] pl-6">
                    ...
                    <div className="flex bg-[#111418] border border-[#1e252e] text-[9px] font-bold p-0.5 h-[28px]">
 <button onClick={() => setSidebarPosition('left')} className={`px-4 h-full flex items-center justify-center transition-colors ${ sidebarPosition === 'left' ? 'bg-[#2d3640] text-[#00ffff]' : 'text-[#8a95a3] hover:text-[#e2e8f0]'}`}>{lang === 'jp' ? '左' : 'LEFT'}</button>
 <button onClick={() => setSidebarPosition('right')} className={`px-4 h-full flex items-center justify-center transition-colors ${ sidebarPosition === 'right' ? 'bg-[#2d3640] text-[#00ffff]' : 'text-[#8a95a3] hover:text-[#e2e8f0]'}`}>{lang === 'jp' ? '右' : 'RIGHT'}</button>
                    </div>
                 </div>

                 {/* Theme Toggles */}
                 <div className="flex items-center gap-2 border-l border-[#1e252e] pl-6">
 <button onClick={() => setThemeMode((prev) => prev === 'dark' ? 'mono' : prev === 'mono' ? 'red' : 'dark')}
                  className="flex items-center gap-2 px-3 h-[28px] bg-[#111418] border border-[#1e252e] text-[9px] font-bold text-[#8a95a3] hover:text-[#e2e8f0] hover:bg-[#2d3640] transition-colors uppercase">
                 ...
                 <div className="flex items-center gap-3 border-l border-[#1e252e] pl-6">
                    {/* Settings Button */}
 <button className="p-1.5 text-[#8a95a3] hover:text-[#e2e8f0] bg-[#111418] hover:bg-[#2d3640] border border-[#1e252e]  transition-colors" onClick={() => setIsGlobalSettingsOpen((prev) => !prev)}
                  title="Global Settings">
                    ...
                    {/* Fullscreen Button */}
 <button className="p-1.5 text-[#8a95a3] hover:text-[#e2e8f0] bg-[#111418] hover:bg-[#2d3640] border border-[#1e252e]  transition-colors" onClick={() => {
 ...
 <button onClick={() => setIsHeaderOpen(false)}
                  className="p-1.5 text-[#8a95a3] hover:text-[#e2e8f0] bg-[#111418] hover:bg-[#2d3640] border border-[#1e252e]  transition-colors"
                  title="Close Header">
 ...
 <button onClick={() => setIsHeaderOpen(true)}
          className="p-1.5 bg-white/90 text-gray-700 hover:text-gray-900 border border-gray-300 hover:bg-white  transition-colors shadow-sm backdrop-blur-sm"
          title="Open Header">
 ...
 <button onClick={(e) => {e.stopPropagation();setIsSidebarOpen(!isSidebarOpen);}}
            className={`group flex items-center justify-center bg-[#4e5d74] border border-[#2d3640] text-[#e2e8f0] hover:bg-[#a3b8cc] hover:text-[#0a1128] transition-all duration-300 shadow-lg h-16 overflow-hidden ${ sidebarPosition === 'left' ? ' border-l-0' : ' border-r-0'}`}
      ...
      {resetConfirmTarget &&
        <div className="absolute inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-[#0a0c10] border border-[#1e252e] p-6  w-[320px] shadow-2xl">
 ...
 <button onClick={() => setResetConfirmTarget(null)} className="flex-1 py-2 text-[10px] font-bold border border-[#1e252e] text-[#8a95a3] hover:text-[#e2e8f0] hover:border-[#4e5d74]  transition-colors">{lang === 'jp' ? 'キャンセル' : 'CANCEL'}</button>
 <button onClick={() => {
              ...
              }} className="flex-1 py-2 text-[10px] font-bold bg-[#d94a38] hover:bg-[#ff5544] text-white  transition-colors">{lang === 'jp' ? 'リセット' : 'RESET'}</button>
            </div>
          </div>
        </div>
        }
